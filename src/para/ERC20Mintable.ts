...
import { ERC20Mintable as ABI } from "../deployment/ABI/ERC20Mintable.json";
...
class ERC20Mintable {
  ...
  mint(spender: string, amount: BigNumber): Promise<TransactionResponse> {
    return this.contract.mint(spender, amount);
  }

  claim(spender: string): Promise<TransactionResponse> {
